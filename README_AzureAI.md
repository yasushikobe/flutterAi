...
- https://azure.microsoft.com/ <br/>に接続し、Azureアカウントを用意します。  

  ![fig1](attach/20221211111656.png)  
  Azureを始めて利用する方は、`Azureを無料で試す`をご利用できます。  
  既にアカウントをお持ちの方は、右上の`サインイン`よりazureにログインを行います。  

## リソースグループを作成する。

``リソースグループ``→``作成``を選択し、新しいリソースグループを作成します。  

![fig1](attach/20221211112208.png)  

key|val
---|---
サブスクリプション|あなたの契約しているサブスクリプションを選択
リソースグループ|任意の名称（今回はAzureAIとした）
リージョン|任意の場所（今回は東京を選択した）

該当項目設定後、`確認及び作成`を選択します。  

![fig1](attach/20221211112611.png)  

`作成`を選択し、リソースグループを作成します。  
